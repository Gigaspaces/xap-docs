...
| `com.gs.transport_protocol.lrmi.min-threads` | XAP maintains a thread pool in the client and server side, that manages incoming remote requests. The thread pool size is increased each time with one additional thread and shrinks when existing threads are not used for `com.gs.transport_protocol.lrmi.threadpool.idle` value duration (300000 ms). This parameter specifies the minimum size of this thread pool. | `1` | Server | Threads | No|
