...
When a cluster topology is replicated, the replication functionality is enabled. More information about topologies is available [here](../overview/the-in-memory-data-grid.html#topologies).
...
When a cluster uses partitioned topology, the data is partitioned across multiple Space instances. More information about topologies is available [here](../overview/the-in-memory-data-grid.html#topologies).
...
This logger corresponds to the security of GigaSpaces components. This includes configuration and runtime execution of security functionality. More information regarding GigaSpaces security is available [here]({{%currentsecurl%}}/).
