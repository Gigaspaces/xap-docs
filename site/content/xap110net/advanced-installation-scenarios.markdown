...
Create an environment variable named `XapNet_<version>_SettingsPath` which points to the settings file path. For example, for that same 11.0 version we would use:


```xml
XapNet_11.0.0.14800_SettingsPath=C:\GigaSpaces\XAP.NET 11.0.0 x64\NET v4.0\Config\Settings.xml
```
...
For example, the XAP.NET v11.0 x64 setup creates the following keys:

```xml
HKLM\SOFTWARE\GigaSpaces\XAP.NET\11.0.0.14800\CLR v2.0.50727\SettingsPath=C:\GigaSpaces\XAP.NET 11.0.0 x64\NET v3.5\Config\Settings.xml
HKLM\SOFTWARE\GigaSpaces\XAP.NET\11.0.0.14800\CLR v4.0.30319\SettingsPath=C:\GigaSpaces\XAP.NET 11.0.0 x64\NET v4.0\Config\Settings.xml
```
...
Use the `XapNetSettingsFile` element to configure the location of the settings file (typically C:\GigaSpaces\XAP.NET-11.0.0-x64\NET v4.0\Config\Default.xml). For example:
...
<configuration>
    ...
    <GigaSpaces>
        <XapNetSettingsFile Path="C:\GigaSpaces\XAP.NET 11.0.0 x64\NET v4.0\Config\Settings.xml"/>
    </GigaSpaces>
</configuration>
```
...
```csharp
    GigaSpacesFactory.Configuration.XapNetSettingsFile.Path = @"C:\GigaSpaces\XAP.NET 11.0.0 x64\NET v4.0\Config\Settings.xml"
```
