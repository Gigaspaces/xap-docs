...
{{% note "Dependency Download by Maven "%}}
...
{{% note %}} The XAP Maven plugin is installed under: `<maven-repository-dir>\org\apache\maven\plugins\xap-maven-plugin`{{%/note%}}
...
{{% note %}} Executing `xap:create` without specifying a template shows a list of available templates and their description.
...
{{% note %}} The archives generated by the common module and its dependencies are added to the `lib` directory of the feeder's and processor's distributables. {{%/note%}}
...
{{% note %}} To run modules, they need to be compiled first.{{%/note%}}
...
{{% note %}} The default values are sufficient when using the data source values generated by the plugin. {{%/note%}}
